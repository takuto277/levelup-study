...
description: PR 作成直後に必ず実行するセルフレビュー。実装者エージェントが自分の PR を github-pr-review と同手順でチェックし、 [must] は修正して push、[should]/[nit] は PR にコメントする。ユーザーへ「PR 完了」報告の前に必須。
---
# 概要
...
3. **PRレビュー状態ラベルを更新しない**（`PRレビュー修正待ち` / `PRレビュー/再レビュー待ち` / `review:まーじOK` は通常レビュー・再チェックの状態ラベルであり、セルフレビュー完了ラベルではない）
4. **`[must]` がある場合**
   - ユーザーへ「マージ可」と報告しない
...
7. **PR 本文の構成を確認する**
   - `gh pr view` で本文を取得し、`## 実装の意図・技術選定` セクションがあるか確認する
   - ない場合: `[must]` 扱い。セクションを追記し `gh pr edit` で更新する
   - 内容チェック:
     - なぜその実装方法・ライブラリ・アーキテクチャを選んだか説明されているか
     - 導入したライブラリの役割が初学者にもわかる言葉で書かれているか
     - 専門用語に初出時の簡単な説明が添えられているか
