...
description: GitHub Issue や実装タスクの対応を依頼され、作業領域の準備、コード変更、 検証、コミット、push、PR 作成までを既存スキルを参照しながら一連で進める時に使う。
---
# 概要

GitHub Issue または実装指示を受け、作業領域の準備、実装、検証、コミット、push、PR 作成までを進める入口スキル。既存スキルがある工程では該当スキルを参照し、専用スキルがない工程だけこのスキル内の手順で進める。基本方針は「作業領域の選択だけ最初に確認し、それ以外は失敗時だけ停止」。ユーザー確認を細かく挟まず、完了まで進める。
...
- Issue は要件定義の正本として扱う。背景、スコープ、受け入れ条件、参照ファイルを実装前に整理する。
- 3 ステップ以上、または API / DB / UI をまたぐ変更では、コード変更前に `docs/tasks/{YYYYMMDD}-{slug}.md` の実行計画書・設計を作成する。
- 計画書には対象 Issue、今回のスコープ / スコープ外、既存コード調査、実装手順、検証計画、互換性リスクを含める。
- 目的、受け入れ条件、影響範囲が十分に分かる場合でも、最初に worktree を作るか、現在の作業ツリーに作業ブランチだけ作るかを確認する。
- ユーザーが「worktree 不要」「ブランチだけで」など作業方式を明示した場合は、その指示を確認済みとして扱い、追加確認せずに従う。
- `main`、`master`、`develop` では直接実装しない。worktree を使わない場合も、必ず作業ブランチを作ってから実装する。
...
- 参照先スキルにユーザー確認を挟む通常運用があっても、このスキルでは作業領域の選択または停止条件に当たる場合だけ確認する。
...
- PR 直後セルフレビュー: `../github-pr-self-review/SKILL.md`
...
- 既存ブランチ、既存 worktree、または現在の未コミット変更が今回の作業専用か判断できず、安全に分離できない。
...
- ユーザーが作業方式を明示していない場合は、実装前に次を短く確認する。

```text
作業用 worktree を作って進めますか？不要なら、現在の作業ツリーで作業ブランチだけ作って進めます。
```

- ユーザーが worktree 作成を選んだ場合:
...
- ユーザーが worktree 不要、またはブランチだけで進めることを選んだ場合:
  - `../git-branch-create/SKILL.md` を読み、ブランチ名の生成ルールを適用する。
  - 現在のブランチが `main`、`master`、`develop` の場合は、`git switch -c <branch>` で作業ブランチを作成してから実装する。
  - 現在のブランチが既に今回の作業専用ブランチだと判断できる場合だけ、そのブランチを再利用してよい。
  - 現在のブランチや未コミット変更が別タスク由来の可能性がある場合は、無関係な変更を取り込まないため停止する。
  - 実装、検証、コミット、push、PR 作成は現在の作業ツリー内の作業ブランチで行う。

## 3. 実装する

- まず関連ファイル、設定、テストを読み、既存パターンに合わせて変更する。
- 3 ステップ以上、または API / DB / UI 横断の変更では、先に `docs/tasks/{YYYYMMDD}-{slug}.md` の実行計画書・設計を作成してからコード変更する。
- 計画書作成後に実装方針が変わった場合は、計画書も同じ PR で更新する。
...
- 作業中の worktree または現在の作業ツリー内の作業ブランチで `../git-commit-message/SKILL.md` を読み、コミットメッセージの生成ルールを適用する。
...
- 作業中の worktree または現在の作業ツリー内の作業ブランチで `../github-pr-create/SKILL.md` を読み、push と PR 作成の手順を適用する。
...
- 変更概要、実行した検証、作成した PR URL、**セルフレビュー結果**を簡潔に返す。
...
- [ ] 作業領域の選択だけ最初に確認し、それ以外は失敗時だけ停止する方針が明記されている
- [ ] worktree 作成、ブランチ名、コミットメッセージ、PR 作成は既存スキルへの参照先が明記されている
- [ ] worktree 不要時は現在の作業ツリーで作業ブランチだけ作る手順がある
...
本リポジトリでは [LEVELUP.md](../LEVELUP.md) を優先する。

- 品質確認: `./scripts/validate-pr.sh`（iosApp 変更時 `--ios`）。[AGENTS.md](../../../AGENTS.md)
- Issue / PR は Actions ではなく `gh` CLI + スキルのみ
- PR 本文ファイル: `docs/tasks/pr-body-{slug}.md`
