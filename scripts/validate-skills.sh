#!/usr/bin/env bash
# .cursor/skills/ の SKILL.md 品質を検証する（u7chan/agent-skills の validate-skills.sh を LevelUp 向けに調整）
set -euo pipefail
...
EXCEPTIONS_FILE="scripts/skill-line-count-exceptions.txt"
...
is_line_count_excepted() {
  local file="$1"
  [[ -f "$EXCEPTIONS_FILE" ]] && grep -qxF "$file" "$EXCEPTIONS_FILE"
}
...
if [[ ! -s "$tmp_dir/skills.actual" ]]; then
  report_failure "no SKILL.md files found under ${SKILLS_DIR}/"
fi

while IFS= read -r skill_file; do
  line_count="$(wc -l < "$skill_file" | tr -d ' ')"
  if (( line_count > MAX_SKILL_LINES )); then
    if is_line_count_excepted "$skill_file"; then
      printf 'WARN: %s has %s lines but is listed in %s\n' \
        "$skill_file" "$line_count" "$EXCEPTIONS_FILE"
    else
...
if [[ ! -f "$README_FILE" ]]; then
  report_failure "${README_FILE} is missing"
else
  grep -Eo '\[[^]]+\]\([^)]+/SKILL\.md\)' "$README_FILE" |
    sed -E 's#.*\(([^)]+)\).*#\1#' |
    while IFS= read -r rel; do
      [[ -z "$rel" ]] && continue
      if [[ "$rel" == "$SKILLS_DIR/"* ]]; then
        printf '%s\n' "$rel"
      else
        printf '%s/%s\n' "$SKILLS_DIR" "$rel"
      fi
