...
- すべての未解決指摘が解消済みで、新しい指摘コメントが 0 件の場合は、`review-state-labels.md` に従って `review:まーじOK` を付与し、他のレビュー状態ラベルを外す。
- `partial` / `unresolved` / `unknown` が 1 件以上ある場合は、`review-state-labels.md` に従って `PRレビュー修正待ち` を付与し、他のレビュー状態ラベルを外す。
